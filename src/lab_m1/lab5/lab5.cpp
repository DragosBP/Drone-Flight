...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(23), glm::vec3(1, 0, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f, 1.5f, 1));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(0, 0.5f, 4));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(241), glm::vec3(1, 0, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(2, 1, 3));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 0.5f, 4));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(57), glm::vec3(1, 0, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.7f, 2.1f, 1.4f));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->GetSpecialKeyState() & GLFW_MOD_SHIFT) {
            ...
            renderCameraTarget = false;
            camera->TranslateRight(deltaX * sensivityOZ);
            camera->RotateFirstPerson_OZ(deltaX * sensivityOZ);
        }
    }
}
